...
func TestInstallScriptsDoNotStartBackgroundServices(t *testing.T) {
	for _, path := range []string{filepath.Join("docs", "install.sh"), filepath.Join("docs", "install.ps1")} {
		data, err := os.ReadFile(path)
		if err != nil {
			t.Fatal(err)
		}
		text := string(data)
		for _, forbidden := range []string{"daemon restart", "daemon start", "Start-Process"} {
			if strings.Contains(text, forbidden) {
				t.Errorf("%s retains background-service behavior %q", path, forbidden)
			}
		}
	}
}
